...
- `app/main-process/`: Electron main-process domain modules. Browser tab/window orchestration lives in `browser-tab-manager.ts`, keyboard shortcut registration in `browser-shortcut-manager.ts`, native app menu and update dialogs in `app-menu-manager.ts`, context menus in `context-menu-manager.ts`, `download-manager.ts` is a compatibility entry for the Download Manager API, `download/` owns download orchestration/controllers/HLS/FFmpeg/runtime state by subdomain, `hls-playback/` owns playback-triggered HLS proxy/probe/capture modules with `hls-playback-capture.ts` kept as the stable adapter, `local-playback/` owns local video range/server/preview helpers with root `local-playback*.ts` files kept as stable adapters, sync worker orchestration lives in `sync-worker-manager.ts`, IPC registration in `ipc-handlers.ts`, IPC payload normalizers in `ipc-normalizers.ts`, settings persistence in `settings.ts`, and release update fetching in `update-checker.ts`.
