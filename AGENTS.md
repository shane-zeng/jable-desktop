...
- `app/main-process/`: Electron main-process domain modules. Browser tab/window orchestration lives in `browser-tab-manager.ts`, keyboard shortcut registration in `browser-shortcut-manager.ts`, native app menu and update dialogs in `app-menu-manager.ts`, context menus in `context-menu-manager.ts`, download orchestration in `download-manager.ts`, local playback range/server/preview helpers in `local-playback.ts`, `local-playback-server.ts`, and `local-playback-preview.ts`, sync worker orchestration in `sync-worker-manager.ts`, IPC registration in `ipc-handlers.ts`, IPC payload normalizers in `ipc-normalizers.ts`, settings persistence in `settings.ts`, and release update fetching in `update-checker.ts`.
