...
下載完成的影片會出現在「本機資料」的「下載清單」tab。已下載項目可以用系統預設播放器開啟，也可以顯示檔案位置、開回來源頁、重試失敗項目，或明確刪除本機檔案。下載清單也提供可保留的多選狀態篩選、批次重試失敗項目、透過下載佇列選單全部暫停/全部繼續/取消等待中項目、刪除選取項目，以及檢視失敗/遺失項目的錯誤紀錄。
