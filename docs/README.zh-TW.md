...
同步期間如果你在 Jable 頁面操作加入或移除，App 會先把操作寫入本機 outbox。預設不會自動送出，會保留在「待同步」讓你手動確認，而且不會先改動一般本機清單；只有重送成功後才會套用到本機資料。若在「設定」裡開啟「同步後自動送出變更」，App 才會在同步頁面抓取完成後依序送回 Jable。
...
- 完整同步加速程度：保守、標準、快速
- 是否在同步後自動送出同步期間累積的收藏與稍後觀看變更
- JSON 匯入、JSON 匯出與本機資料庫位置
