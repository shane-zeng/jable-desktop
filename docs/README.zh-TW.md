...
下載功能需要本機安裝 FFmpeg。第一版 App 不會內建 FFmpeg，也不會自動替你安裝。
...
下載完成的影片會出現在「本機資料」的「下載清單」tab。已下載項目可以用系統預設播放器開啟，也可以顯示檔案位置、開回來源頁、重試失敗項目，或明確刪除本機檔案。
...
- FFmpeg 狀態、重新檢查、手動選擇 FFmpeg，以及下載資料夾位置
