...
- Main-process browser sync and diagnosis requests are sent to `app/webview-preload.ts` through request/response IPC channels. Pure webview preload helper behavior for pager/AJAX URL parsing, retry/backoff, metric parsing, page numbers, and video path keys lives in `app/browser/webview-preload-helpers.ts`; focused preload runtime concerns live under `app/browser/webview-preload/`. Do not call embedded page functions through injected JavaScript strings.
...
- `app/webview-preload.ts`: preload composition root, scraper, sync flow, and collection action observer injected into each embedded Jable `WebContentsView`.
