...
- Main-process browser sync and diagnosis requests are sent to `app/webview-preload.ts` through request/response IPC channels. Browser-side sync orchestration and deferred remote replay live in `app/browser/webview-preload/browser-sync.ts`. Pure webview preload helper behavior for pager/AJAX URL parsing, retry/backoff, metric parsing, page numbers, and video path keys lives in `app/browser/webview-preload-helpers.ts`; focused preload runtime concerns live under `app/browser/webview-preload/`. Do not call embedded page functions through injected JavaScript strings.
