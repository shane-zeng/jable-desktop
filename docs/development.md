...
For HLS playback diagnostics, start Electron with `JABLE_HLS_PROBE=1`. The probe observes Jable-session HLS playlist and segment requests from the main process, prints `webContentsId`, Browser tab id, resource type, host, a short path hash, response status, cache state, and the canonical Jable video URL when available. It does not intercept, redirect, download, or log full HLS URLs. Segment logs are capped by default; set `JABLE_HLS_PROBE_VERBOSE=1` during short manual sessions when every segment request is needed. Proxy and capture internals keep high-frequency token, request, asset, segment, and prefetch logs silent by default; set `JABLE_HLS_VERBOSE=1` only during short manual sessions when those internal lifecycle logs are needed. The formal playback-triggered download implementation lives under `app/main-process/hls-playback/`: `capture.ts` is the composition entrypoint, `proxy-server.ts` owns loopback token routing and playlist rewriting, `ipc.ts` owns renderer IPC channel registration and payload parsing, `capture-writes.ts` owns segment file writes and background prefetch, `proxy-headers.ts` owns request/response headers, `probe.ts` owns debug request observation, `helpers.ts` owns pure URL/rewrite helpers, and `shared.ts` owns shared types and environment gates. `research.ts` is a legacy research entrypoint that delegates to the production capture installation so old imports do not keep a second proxy implementation alive. The Settings option **Auto-download while playing** is off by default; when enabled, the app rewrites page `fetch`/`XMLHttpRequest` `.m3u8` calls to short loopback token URLs, fetches the original playlist in main, rewrites segment, key, and nested playlist URIs to short loopback token URLs, and uses one managed segment cache for both playback and background completion. Playlist preload may be proxied before playback starts, but download records and managed segment writes begin only after webview preload observes a real page `<video>` play/playing event. Player requests and background prefetch share the same in-flight segment fetch, so media segments are not fetched twice for the same video. Pause, cancel, and delete stop playback-capture writes; delete also suppresses the still-open token from recreating a record. After all media segments are cached, the normal resume/remux path assembles the MP4 from local segment files. For manual diagnostics, `JABLE_HLS_PROXY=1` enables proxying without persisted setting state and `JABLE_HLS_CAPTURE=1` enables capture regardless of the persisted setting. The proxy intentionally avoids `webRequest` redirects because Chromium rejects CORS XHR external redirects to loopback.

The app also writes always-on local diagnostics under Electron `userData/logs`, not the platform-default macOS Logs directory, so packaged and test user data remain self-contained across macOS, Windows, and Linux. `app/main-process/diagnostics/logger.ts` writes JSONL entries with one append per event, rotates at 5 MB per file, keeps JSONL logs for 14 days, enforces a 100 MB managed-log cap, and keeps only the newest 10 crash dump files under `userData/logs/crashes`. Electron crash reporting is local-only with `uploadToServer: false`. Diagnostics cover main process startup/shutdown, database open/close, settings updates, IPC handler failures, renderer/webview global errors, WebContents crashes or failed loads, sync worker/browser-tab process exits, and download/native/FFmpeg/file phases. The sanitizer masks full remote URLs, HLS playlist/segment/key details, cookies, authorization headers, tokens, and user/download-root paths before writing. Settings > Data exposes **Open Log Folder** and **Clear Diagnostics**; clear deletes only managed diagnostics files and tolerates temporary file locks, especially on Windows.

When adding diagnostics in production runtime code, route handled failures through the diagnostics module instead of direct `console.*` output. Main-process modules should receive `DiagnosticsLogger` or narrow `event` / `errorEvent` callbacks from their composition root; webview preload code should report through the fire-and-forget diagnostics IPC bridge; renderer workflows should use `reportRendererError()` for handled failures while the main process continues to capture global renderer and webview failures. Diagnostics calls must stay best-effort and sanitized: wrap logger callbacks from catch/finally cleanup paths when needed, avoid logging full remote URLs or filesystem roots, and never let a failed log write, IPC send, or callback create a new crash path. The diagnostics logger's internal fallback console is the final safety net, not the normal logging API.

Cross-platform file cleanup must assume Windows can deny access to files that macOS removes immediately. Chromium media playback, FFmpeg child processes, Explorer thumbnails, antivirus/indexers, and OS timing can keep `.mp4`, `.mp4.part`, `.segments`, `.preview`, and `.preview.tmp` paths locked after a download, playback capture, preview generation, or delete request has otherwise finished. Treat `EPERM`, `EACCES`, and `EBUSY` as expected operational outcomes: preserve the download record when the primary managed MP4 cannot be deleted, quarantine working directories with a deletion suffix when a rename is possible, retry quarantined cleanup on startup and download-root changes, and emit diagnostics for non-`ENOENT` cleanup failures. Avoid platform-specific path assumptions by using the existing managed-path and Windows-safe filename helpers rather than ad hoc string checks.
...
- App backup is separate from collection JSON. Settings backup stores normalized app settings plus renderer-local UI preferences; full backup also snapshots logical SQLite data through `exportBackupData()`: `videos`, all `collection_items`, `sync_states`, and `download_assets`. It excludes `sync_operations`, FTS tables, cookies, browser session, main-window state, diagnostics, and media files. Full import merges rows, clears `sync_operations`, pauses active download states, and applies settings only after data import succeeds.
...
- Rust-owned data-engine invariants are covered directly in `native/local-data-engine/src/tests.rs`. Any change to migrations, FTS/search tokenization, sync visibility, sync operation reduction, outbox state transitions, pending remote grouping, resolved/superseded handling, JSON import/export, or app backup import/export should add or update Rust tests there in addition to the shared Node data-engine contract tests.
...
- User-facing app settings are stored in `settings.json` under Electron `userData` through `app/main-process/settings.ts`. Shared limits and defaults live in `app/app-contract.ts`. Keep `app/types/jable.ts`, `app/app-contract.ts`, `app/main-process/settings.ts`, `app/main-process/app-backup.ts`, `app/preload.ts`, main IPC handlers, `SettingsPanel.vue`, and settings/app-backup tests aligned when adding or changing settings or backup settings import behavior.
- Main app window state is stored separately as `main-window-state.json` under Electron `userData` through `app/main-process/window-state.ts`. The snapshot includes normal `x`, `y`, `width`, `height`, and maximized state. Startup placement is resolved by `app/main-process/window-options.ts`, which clamps restored bounds to visible display work areas and app minimums.
...
- `app/main-process/`: main-process domain modules for browser runtime composition, BrowserView tabs, keyboard shortcuts, browser session restore/save, browser preload request bookkeeping, active-origin fallback, diagnostics logging/crash dump management, app/native menus and update dialogs, app local/export/documentation actions, context menus, app-level download shutdown gating, download orchestration, HLS playback proxy/probe/capture modules, local playback range/server/preview helpers, sync workers, IPC registration, IPC payload normalizers, settings persistence, app window state persistence, and release update fetching.
...
- `test/node/diagnostics-logger.test.js`: JSONL diagnostics formatting, sanitization, rotation, retention, total-size pruning, crash dump pruning, and clearing behavior.
...
- Export settings backup and full app backup from Settings > Data. Import the full backup into a test profile and confirm settings/UI preferences are applied, collection rows and download records appear, Pending Sync is empty, and downloaded media files are not expected to be present.
