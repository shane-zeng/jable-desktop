...
- The Rust data engine owns persisted outbox state. Deferred rows move through `pending`, `applied`, `failed`, `blocked`, `resolved`, and `superseded`; the webview only performs Jable AJAX with the current cookie/session. Automatic replay is controlled by Settings and is off by default. When replay is off, deferred remote operations remain only in Pending Sync and do not alter normal local list visibility until a Jable AJAX success marks the group applied or resolved. When enabled, replay still runs in operation order and stops after the first failed operation, marking later pending rows as blocked. The main process replays the outbox one operation at a time so it can emit `sync-queue-progress` updates and keep the renderer progress bar accurate.
