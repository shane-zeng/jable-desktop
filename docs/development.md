...
- `app/main-process/`: main-process domain modules for BrowserView tabs, keyboard shortcuts, app/native menus and update dialogs, context menus, download orchestration, HLS playback proxy/probe/capture modules, local playback range/server/preview helpers, sync workers, IPC registration, IPC payload normalizers, settings persistence, and release update fetching.
