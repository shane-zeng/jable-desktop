...
| Toggle download sidebar | `Command+Shift+D`                                             | `Ctrl+Shift+D`                        | Toggles the Browser download progress sidebar only when the current top-level view is Browser and the sidebar feature is enabled in Settings; when disabled, the app does not intercept the shortcut. |
