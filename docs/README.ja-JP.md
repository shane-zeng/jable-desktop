...
ダウンロード済み動画はローカルデータの「ダウンロード一覧」タブに表示されます。完了した項目はシステム既定のプレイヤーで開く、ファイルの場所を表示する、Jable の元ページを開く、失敗項目を再試行する、またはローカルファイルを明示的に削除できます。
...
- FFmpeg の状態、再確認、FFmpeg の手動選択、ダウンロードフォルダーの場所
