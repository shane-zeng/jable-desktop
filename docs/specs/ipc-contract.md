...
- Browser-tab preload may invoke the internal `hls:playlist-proxy-url` IPC from a trusted Jable video page with the playlist URL and current page metadata. It also sends the internal `hls:playback-started` notification when the page `<video>` starts playing, including whether that playback followed a recent user gesture in the same page load. Main returns unavailable while `autoDownloadOnPlayback` is off, unless a development HLS proxy/capture environment flag explicitly enables diagnostics. Settings-driven playback-triggered capture requires that user-initiated playback signal; restored tabs, reload autoplay, or script-started playback must not create or resume a managed download.
