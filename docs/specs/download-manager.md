...
- The webview preload automatically replaces the current Jable video page's primary `<video>` source when local playback is already available during that page load. If this page-load replacement happens after remote playback has already started, the preload preserves the current time and resumes playback only when the video was already playing. If replacement fails or no local playback source is available, the original Jable page playback remains available for that page load; a later download completion does not switch the already-open page to local playback until the page is opened or refreshed again. If the active local playback file is deleted while the video page is open, the preload reloads the page instead of trying to partially restore the Jable player runtime.
...
- With the setting on, the webview preload may proxy Jable page `.m3u8` requests through app-owned loopback token URLs. The proxy is allowed to serve page preload traffic before playback starts, but it must not create a download record or write segment files until the preload observes a real `<video>` `play` or `playing` event for the current trusted Jable video URL after a recent trusted primary-button, touch, or keyboard gesture inside the current video player in that page load. Synthetic page events, recommendation-list clicks, middle-clicks, restored tabs, page reloads, and site/script autoplay must not start or resume playback-triggered downloads by themselves.
...
- Collection tabs can filter to downloadable rows. This filter includes rows with no download record, `paused`, `failed`, and `missing` records, and excludes `queued`, `downloading`, and `ready` records.
- Collection tabs can filter to downloaded rows. This filter includes only rows with a `ready` download record.
