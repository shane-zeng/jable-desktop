...
- Pure webview preload parsing, retry, page, and URL helper behavior lives in `app/browser/webview-preload-helpers.ts`; browser sync, video metadata, and focused preload runtime concerns live under `app/browser/webview-preload/`.
...
- `app/browser/webview-preload/video-metadata.ts`
