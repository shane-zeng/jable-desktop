...
- Diagnostics/logging changes: run diagnostics logger tests, IPC guardrail tests, renderer Settings/App tests, typecheck, and Electron smoke when startup paths, crash paths, or preload exposure change. Verify handled main-process and preload failures use diagnostics channels instead of direct production `console.*`, and verify diagnostics callbacks are best-effort in catch/finally cleanup paths.
- Download List, FFmpeg, native download engine, speed modes, failure metadata, pause/resume, or download pipeline changes: run download Node tests, `npm run rust:ci`, renderer component/composable tests, typecheck, lint, and renderer build. Include Windows-style file-lock cases for primary MP4 deletion, `.mp4.part` cleanup, `.segments` quarantine/retry, preview cache cleanup, and `EPERM`/`EACCES`/`EBUSY` handling when the changed code touches file cleanup. Run Electron smoke tests only when the change also touches Electron startup, preload exposure, protocol/session setup, or main/preload IPC registration wiring.
...
- Settings can toggle playback-triggered auto-download; it defaults off, and only video playback after a trusted player gesture should create a Download List record and write managed segment files. Page preload of a playlist, synthetic page events, recommendation-list clicks, middle-clicks, restored-tab autoplay, reload autoplay, or script-started playback must not start a persisted download.
